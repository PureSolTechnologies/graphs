...
public interface StateModel<S extends State<S, T>, T extends Transition<S, T>> extends Graph<S, T> {
     ...
     * point of the first transition to be made. The first transition may be on of
     ...
     * model is finished and cannot be changed anymore. There are different possible
     * end states due to the normal ends, aborts and errors.
     * 
     * These are the states which must not have a transition! All other states need
     * to have at least one.
     * 
     * @return A {@link Set} of state is returned which mark the end state of the
     *         state model. The return value must not be null, but the set might be
     *         empty.
     */
     ...
     * 
     * @return The current {@link State} is returned. The return value must not be
     *         null, because the state model has to be in one state.
     */
    public S getState();
     ...
     * 
     * @param transition
     *            is the transition to be checked for validity at the current state.
     ...
     * @return A transition is returned in case there is a valid transition to the
     *         given state. <code>null</code> is returned in case there is not valid
