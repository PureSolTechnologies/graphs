...
public interface State<S extends State<S, T>, T extends Transition<S, T>> extends Vertex<S, T> {
     ...
     * 
     * The list of transition must not be null! An empty is list is to be returned
     * if no transition can be found!
     * 
     * @return A {@link Set} of valid transitions is returned. The return value must
     *         not be null, but an empty set is allowed.
     */
